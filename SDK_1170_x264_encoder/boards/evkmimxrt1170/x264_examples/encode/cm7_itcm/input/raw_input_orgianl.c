    ...
    //        while( i_frame > h->next_frame )
     //       {
                ...
                //    return -1;
                 ...
                 memcpy(pic->img.plane[0], raw_data_in+i_frame * h->frame_size, h->plane_size[0]);
                 memcpy(pic->img.plane[1], raw_data_in+h->plane_size[0]+i_frame * h->frame_size, h->plane_size[1]);
                 memcpy(pic->img.plane[2], raw_data_in+h->plane_size[0]+h->plane_size[1]+i_frame * h->frame_size, h->plane_size[2]);
               // for( int i = 0; i < pic->img.planes; i++ ){
                //      memcpy(pic->img.plane[i], raw_data_in+i_frame * h->frame_size, h->plane_size[i]);
                 //   memcpy(pic->img.plane[i], raw_data_in+i* h->plane_size[i-1]+i_frame * h->frame_size, h->plane_size[i]);
               // }
      //  
      //          h->next_frame++;
      //      }
