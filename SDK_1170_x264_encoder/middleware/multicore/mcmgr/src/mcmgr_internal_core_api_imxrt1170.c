...
#if defined(FSL_FEATURE_MU_SIDE_B)
    if (ANADIG_MISC->MISC_DIFPROG == 0x1170A0)
    {
        //*(uint32_t*)0x400e404c = (uint32_t)bootAddress >> 7;
        *(uint32_t *)&IOMUXC_GPR->RESERVED_0[0] = (uint32_t)bootAddress >> 7;
    }
    else
    {
        IOMUXC_LPSR_GPR->GPR26 = IOMUXC_LPSR_GPR_GPR26_CM7_INIT_VTOR((uint32_t)bootAddress);
    }
...
{
#if defined(FSL_FEATURE_MU_SIDE_A)
    if (0 == (OCOTP->FUSE022 & BT_CORE_SEL_BIT))
    {
