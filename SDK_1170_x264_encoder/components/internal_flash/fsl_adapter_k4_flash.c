...
static hal_flash_status_t HAL_FlashProgramAdaptation(uint32_t dest, uint32_t size, uint8_t *pData)
{
    return HAL_FlashGetStatus(FLASH_Program(&s_flashConfig, dest, (const uint32_t *)pData, size));
}
...
{
    (void)margin;
    return HAL_FlashGetStatus(FLASH_VerifyErasePhrase(&s_flashConfig, start, lengthInBytes));
}
