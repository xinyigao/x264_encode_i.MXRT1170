...
static bool GT911_VerifyFirmware(const uint8_t *firmware)
{
    return ((firmware[GT911_REG_CONFIG_VERSION - GT911_CONFIG_ADDR] != 0) &&
            (GT911_GetFirmwareCheckSum(firmware) == firmware[GT911_CONFIG_SIZE - 2]));
}
...
status_t GT911_Init(gt911_handle_t *handle, const gt911_config_t *config)
{
    ...
    gt911Config[GT911_CONFIG_SIZE - 2] = GT911_GetFirmwareCheckSum(gt911Config);
    gt911Config[GT911_CONFIG_SIZE - 1] = 1; /* Mark the firmware as valid. */
...
status_t GT911_GetSingleTouch(gt911_handle_t *handle, int *touch_x, int *touch_y)
{
    ...
    if (kStatus_Success == status)
    {
        ...
        {
            status = kStatus_TOUCHPANEL_NotTouched;
        }
    }
    else
    {
